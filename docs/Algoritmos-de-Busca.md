...
Os algoritmos de busca têm como objetivo verificar ou recuperar um elemento de alguma estrutura de dados. 

De acordo com o tipo de ordenação, podemos classificar esses algoritmos em duas catagorias:

- **Busca Sequencial**:
  - Nesse tipo a estrutura de dados é percorrida sequencialmente e cada elemento é verificado. 
...
- **Busca por Intervalo**:
  ...
  - Sua performance é muito mais eficiente que a `busca linear`, uma vez que o centro da estrutura é visitado repetidamente, além do espaço de pesquisa ser divido pela metade. 
...
O objetivo da busca (ou pesquisa) é encontrar uma ou mais ocorrências de registros com valores iguais ao valor procurado. Existem vários métodos de busca e a sua escolha depende, principalmente:
...
A complexidade de tempo desse algoritmo é `O(n)`, o que justifica o fato de ser raramente utilizado de forma prática. Outros algoritmos de busca como: `busca binária` e `tabela hash`, são mais usados pois permitem uma busca mais rápida.
...
Uma abordagem simples para resolver esse problema é fazer uma `busca linear`, porém a complexidade de tempo desse algoritmo é extremamente alta `O(n)`. Outra abordagem utilizada é `busca binária`.

- Buscamos em uma matriz classificada dividindo repetidamente o intervalo de pesquisa pela metade.
- Se o elemento foi encontrado, termine abusca. 
...
Esse método de busca é eficiente para um vetor ordenado. Podemos dizer que ele é semelhante ao método que usamos para procurar uma palabra no dicionário.
...
Para resolver esse problema podemos adotar uma implementação **recursiva** ou uma implementação **iterativa**.
...
- Esse algoritmo segue o paradigma **Dividir para Conquistar**.
